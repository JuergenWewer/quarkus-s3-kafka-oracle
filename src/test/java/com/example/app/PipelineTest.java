...
@QuarkusTest
@QuarkusTestResource(TestInfraResource.class)
public class PipelineTest {
    ...
    static OracleContainer oracle = new OracleContainer(DockerImageName.parse("gvenzl/oracle-xe:21-slim"))
            .withUsername("userEins").withPassword("oracle");
...
//    }
    ...
    static void setUpS3() {
        ...
        String endpoint = config.getValue("app.s3.endpoint", String.class);
        String region   = config.getValue("app.s3.region", String.class);
        String access   = config.getValue("app.s3.access-key", String.class);
        String secret   = config.getValue("app.s3.secret-key", String.class);
        bucket          = config.getValue("app.s3.bucket", String.class);
...
//        kafka.stop();
    ...
    public void endToEnd() {
        ...
        try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(consumerProps)) {
            ...
            assertThat("ID from names-out", id != null);
        }

        var listed = s3.listObjectsV2(ListObjectsV2Request.builder().bucket(bucket).build());
        assertThat("at least one object",listed.hasContents());
    }
}
